...
class ApiService {
          ...
          try {
            ...
            const response = await Promise.race([
              ...
              new Promise((_, reject) => 
                setTimeout(() => reject(new Error('REISSUE_TIMEOUT')), 5000)
              )
            ]) as any;
            ...
            // 만료된 refresh token인 경우 또는 타임아웃인 경우 로그인 화면으로 이동
  ...
  async getProfile(token: string): Promise<{ user: Member }> {
    ...
    try {
      ...
      console.log('Profile 응답 성공:', response.data);
      ...
      console.error('=== API: getProfile 실패 ===');
  ...
  async registerFCMToken(tokenData: FCMTokenRequest): Promise<FCMTokenResponse> {
    try {
      ...
      if (!tokenData.token || !tokenData.deviceId) {
        throw new Error('FCM 토큰과 디바이스 ID는 필수입니다.');
      }
      
      // 기존 서버 엔드포인트 사용
      const response = await this.axiosInstance.post('/token/register', tokenData);
      ...
      if (error.response) {
        ...
        if (status === 500) {
          console.error('서버 내부 오류 (500):', data);
          throw new Error('서버 내부 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
        } else if (status === 400) {
          console.error('잘못된 요청 (400):', data);
          throw new Error('잘못된 요청입니다. 입력 정보를 확인해주세요.');
        } else if (status === 401) {
          console.error('인증 실패 (401):', data);
  ...
  async deleteFCMToken(deviceId: string): Promise<FCMTokenResponse> {
    try {
      const response = await this.axiosInstance.delete(`/token/delete`, {
        data: { deviceId }
      });
      return response.data;
    } catch (error) {
