...
### 스플래시 화면
1. **앱 로고 표시**: Garabu 브랜딩과 태그라인
2. **로딩 애니메이션**: 부드러운 페이드인과 스케일 애니메이션
3. **진행률 표시**: 로딩 진행 상황을 시각적으로 표시
4. **상태 메시지**: 현재 진행 중인 작업 설명
5. **인증 상태 확인**: 저장된 토큰으로 자동 로그인 시도

### 자동 라우팅
- **로그인된 상태**: 메인 탭 화면으로 자동 이동
- **로그인되지 않은 상태**: 로그인 화면으로 자동 이동
- **에러 발생**: 적절한 에러 메시지와 함께 로그인 화면으로 이동
