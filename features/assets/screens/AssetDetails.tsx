...
export default function AssetDetailModal() {
  ...
  const handleBalanceUpdate = async () => {
    ...
    const amount = parseFormattedNumber(balanceAmount);
    if (amount <= 0) {
    ...
    try {
      ...
      await updateAssetBalance(asset.id, amount, balanceOperation, token);
            ...
            <View style={styles.balanceInputContainer}>
              ...
              <View style={styles.amountContainer}>
                <TextInput
                  ...
                  value={formatAmountInput(balanceAmount)}
                  onChangeText={(text) => {
                    const formattedText = formatAmountInput(text);
                    setBalanceAmount(formattedText);
                  }}
                  placeholder="0"
                  placeholderTextColor={colors.tabIconDefault}
                  keyboardType="numeric"
                />
                {balanceAmount && parseFormattedNumber(balanceAmount) > 0 && (
                  <ThemedText style={[styles.amountInWords, { color: colors.icon }]}>
                    {formatKoreanAmount(balanceAmount)}
                  </ThemedText>
                )}
              </View>
            </View>
